...
export class AskService {
  ...
  /**
   * Single normalization for all channels (Web UI, WhatsApp, etc.) before POST /agent.
   * Intent routing uses embeddings; casing and stray whitespace change vectors and can
   * flip nearest-neighbor intent (e.g. action vs knowledge) for the same user intent.
   */
  private normalizeAgentMessage(message: string): string {
    let s = String(message ?? "")
      .trim()
      .replace(/\s+/g, " ")
      .toLowerCase();
    // Trailing punctuation changes embeddings and can flip intent (e.g. action vs knowledge).
    s = s.replace(/[.,!?;:]+$/g, "").trim();
    return s;
  }

  /**
   * Sarvam TTS returns a disk path like `audio/tts_*.mp3`.
   * Public URL must be `/audio/tts_*.mp3` (do not prefix `/audio/` again).
   */
  private ttsPathToPublicUrl(ttsOutput: string): string {
    const s = String(ttsOutput || "")
      .trim()
      .replace(/\\/g, "/");
    if (!s) return "/audio/unknown.mp3";
    const base = s.replace(/^\/?audio\//, "");
    return `/audio/${base}`;
  }
  ...
  async ask(question: string, options?: AskAgentOptions) {
    const userId = options?.userId ?? "ui_user";
    const role = options?.role ?? "store";
    const message = this.normalizeAgentMessage(question);

  try {
    const url = this.agentUrl();
    console.log("[ASK][pipeline][2/nest:service] AskService.ask — received question");
    if (message !== String(question ?? "").trim()) {
      console.log("[ASK][pipeline][2/nest:service] normalized for /agent (trim/space/case):", message);
    }
    console.log("[ASK][pipeline][3/nest:service] Calling ai-agent POST", url, {
      user_id: userId,
      role,
...
async handleVoice(
  ...
  // 4️⃣ Semantic search (DB optional — never fail the whole voice flow)
  ...
  try {
    ...
    console.warn("[handleVoice] semantic_cache search skipped:", msg);
  }
  const best = candidates?.[0];
...
if (best && best.distance < 0.45) {
  ...
  return {
    audio_url: this.ttsPathToPublicUrl(audioFile),
  };
}
...
const audioFile = await this.sarvam.tts(answerText, "en-IN", gender);
  ...
  const answerAudioPath = this.ttsPathToPublicUrl(audioFile);
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : String(e);
    console.warn("[handleVoice] semantic_cache store skipped:", msg);
  }
